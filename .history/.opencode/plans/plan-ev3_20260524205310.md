...
**Al guardar:**
Actualmente el `onSubmit` pasa solo los datos del schema Zod. Hay que modificar el llamado a `addTransaction` para que también incluya `photoUri` y `location`:
...
} as CreateTransactionInput & { photoUri?: string; location?: Transaction['location'] })
```
...
Mismo patrón que create.tsx, pero:

- Si `transaction.photoUri` existe, se pasa como valor inicial. `useImagePicker` acepta un `initialUri` opcional para mostrar preview.
- Si `transaction.location` existe, se muestra como coordenadas iniciales. El componente muestra el valor antes de que el usuario interactúe.
- `defaultValues` del formulario se expande para incluir `photoUri` y `location`.
- Al guardar, se envían junto con `updateTransaction`.
...
| `hooks/useImagePicker.ts` | **Crear** | Hook completo de cámara/galería |
...
| `app/(tabs)/transactions/[id]/edit.tsx` | Modificar | Secciones de foto + ubicación en edición |
